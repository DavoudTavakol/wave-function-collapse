...
\begin{abstract}
In dieser Bachelorarbeit wird auf Funktionsweise des Wave-Function-Collapse {(WFC)} Algorithmus von Maxim Gumin eingegangen und wie dieser für Textursynthesen sowie zur Erstellung
von Prozedural generierten Content verwendet werden kann.
Dazu wird zuerst auf gängige Textursynthese Algorithmen eingegangen und wie fern sich WFC von diesen Unterscheidet.
...
Danach wird auf die unterschiedlichen Implementierungen der jeweiligen Algorithmen aufgezeigt wie z.B. die Blockweise Generierung der Model Synthese das Overlapping Model von Gumin.
Zum Schluss wird das Overlapping Model nochmal detailliert erklärt und weshalb gerade diese Implementation so großen aufsehen erlangt hat und welche Anwendungsfälle mit WFC
...
Later, the model synthesis of Paul Merrell is used to show similarities between the two algorithms and to compare them.
...
Afterwards, the different implementations of the respective algorithms are pointed out, such as the modifying on blocks generation of the model synthesis and the overlapping model of Gumin.
Finally, the Overlapping Model is explained in detail and why this implementation has gained so much attention and which use cases are generally possible with WFC.
\end{abstract}
...
Anhand von einem Beispiel wird im Detail die Funktionsweise zum Lösen solcher CSP's aufgezeigt.
...
während in einer in einer Markow-Kette höherer Ordnung, ein Markov-Netzwerk, jeder Zustand von seinen Nachbarn in einer von mehreren Richtungen abhängt. \cite{wiki:Markov_model}
...
wird diese hier nicht weiter behandelt da sie für das weitere Verständnis dieser Arbeit keine Relevanz hat und nur eine weitere Synthesemethode darstellen soll.
