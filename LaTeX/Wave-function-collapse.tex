...
Jedes lokale Muster ist immer im Input wiederzufinden {(siehe Abb. 4.1)}. {[1][3][4]}
...
In Abbildung 4.5 können wir klar die Schwachstellen von anderen Textursynthesen erkennen.\\
...
dass es sich hierbei bereits um die \glqq Simple Tile\grqq{} Version des WFC Algorithmus handelt.
...
Laut Paul Marrell gibt es mehrere Unterschiede in der Implementierung.\\
...
In Abbildung 4.12 sehen wir einen Ausschnitt einer einzelnen Zelle mit allen möglichen Patches die an seinen benachbarten $x,y$-Offsets erlaubt sind.
...
Allerdings funktioniert $N = 1$ in diesem Fall nicht, da jeder Pixel zu einem "Patch" wird die wir selbstverständlich nicht mehr überlappen können.
...
%wichtige punkte!
It is also important to understand how model synthesis / WFC compares to other texture synthesis algorithms. This is
discussed in greater detail in [Merrell 2009], but I will summarize the main points here. Many of the textures shown
in Figures 2 - 10 could also be generated by other state-of-the -art texture synthesis algorithms. The main advantage
that model synthesis / WFC has is it is better at forming closed loops. On the other hand, if you used a photograph
as an input texture, many texture synthesis algorithms would work fine, but model synthesis / WFC would not. This
is because they require the input image to be self-similar. In most photographs, the image patches all have slightly
different colors. Model synthesis and WFC require pixel art style images that only use a few colors.


%vor und nachteile overlapping, tiled, und modell synthese nicht vergessen!

% Mal schauen was ich mit diesem teil mache!
In Abschnitt 4.0 wurde bereits auf mögliche Vorteile einer pixelgenauen Textursynthese hingewiesen.
