...
Zum Schluss wird das Overlapping Model nochmal detailliert erklärt und weshalb gerade diese Implementation so großes aufsehen erlangt hat und welche Anwendungsfälle mit WFC
...
For this purpose, we will first discuss common texture synthesis algorithms and how WFC differs from them.
...
Die Aufgabe von Algorithmen zum Lösen von CSP's \textit{(solver)} ist es einen Zustand \textit{(State)} zu finden in denen alle constraints erfüllt sind und jedem Objekt nur noch ein Wert zugeordnet ist.
...
Während dem Prozess einen gültigen State für das CSP zu finden, so gibt es immer Situationen in dem es mehrere gültige Optionen für einen der Objekte gibt.
Wenn so eine Situation auftritt, dann haben verschiedene Solver verschiedene Ansätze.
Einige Algorithmen wähle zufällig eine der möglichen Werte von den momentan zulässigen Optionen.
...
In so einem Fall gibt es Rücksetzverfahren \textit{(Backtracking)} bei dem der Algorithmus zu seinem letzten Ergebnis zurückfällt und einen anderen Wert für die Variable setzt, um aus dem
ungültigen Zustand zu kommen und es neu zu versuchen.
...
Es wurde bereits erwähnt das Gumin's Wave-Function-Collapse Algorithmus sehr von Paul Merrell's diskreter Model Synthese inspiriert ist.
...
Dadurch konnte er neue Objekte sowohl in 2D als auch im 3D-Raum generieren.
...
Auch die Komplexität des Inputs kann dazu beitragen das WFC nicht zu einem Ergebnis kommt.
Obwohl das ebenfalls der Fall für Merrell's Implementierung ist, so ist es bei der Model Synthese wesentlich wahrscheinlicher das ein gültiges Ergebnis zustande kommt,
...
Das bedeutet nicht das WFC von so einer Implementierung nicht profitieren würde, vor allem wenn man WFC im 3D-Raum verwenden möchte oder größere Outputs erzielen möchte.
...
Neben weiteren Unterschieden wie die Laufzeit und der Erweiterbarkeit der beiden Algorithmen so gibt es noch einen weiteren Unterschied den Gumin's WFC implementiert.
...
Das hat den Vorteil das die Patches ganz einfach aus dem Input errechnet werden können.
...
In diesem Beispiel wurden mit $N = 2$, also eine Fenstergröße von 2 Pixel, die einzelnen Module aus dem Input generiert.
...
außer $N = 1$ repräsentiert eine Menge an Pixeln.
Bei zu kleiner Fenstergröße kann es sein, das der Output nicht dem Input ähnelt.
Andersrum bei einer zu großen Fenstergröße kann der Output dem Input zu ähnlich sein, da ganze Gruppen von Strukturen einfach kopiert und eingefügt werden.
...
Andersrum hingegen wenn statt einem lokalen und stationären {(siehe Abbildung 2.3)} Input bspw. ein Foto verwendet wird,
...
als gemischte Pixelwerte aller möglichen Optionen innerhalb der Zellen darstellt, dann führt das zu einer schön anzusehenden Generierung des Outputs.
...
Joseph Parker war einer der ersten, der WFC verwendet hat.
