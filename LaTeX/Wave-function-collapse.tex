...
Nach D.Gomathi und Rajvi Shah {([AAA], S.1)}, kann die Textursynthese wie folgt definiert werden.
\newline
...
\begin{itemize}
    ...
    \item Dasselbe Muster soll nicht mehrfach in der neuen Textur vorkommen. {([AAA], S.2)}
\end{itemize}
...
Lokal ist ein Image dann, wenn jeder Pixel allein von seinen Nachbarn bestimmt werden kann.{[AAA]}

\begin{figure}[H]
    \centering
    \subfloat[][]{\includegraphics[width=0.45\linewidth]{images/Textur-nicht-stationärlokal.JPG}}%
    \qquad
    \subfloat[][]{\includegraphics[width=0.45\linewidth]{images/Textur-stationärlokal.JPG}}%
...
Diese Attribute bezeichnet man als Stationär und Lokal.{[AAA]}
Im Folgenden die Funktionsweise eines Algorithmus basierend auf Markov Random Fields nach Efros und T. Leung: {[BBB]}
...
\begin{itemize}
    ...
    \item Dieser Prozess wiederholt sich so lange, bis alle nicht bekannten Pixel generiert wurden.{([AAA], S.4)}
\end{itemize}
...
Hierbei werden aus dem Input-Image mehrere Output-Images in verschiedenen Auflösungen mithilfe von Glättung und Downsampling generiert {(siehe Abbildung 2.5 (a))}. {[EEE]}
\newline
...
Dabei wird die Gleichverteilung mithilfe der Grauwertverteilung berechnet, damit der gesamte zur Verfügung stehende Wertebereich optimal ausgenutzt wird.{[FFF]}
...
sodass die Stichproben gleichmäßig auf dem Intervall {[0, 1]} verteilt sind.{[EEE]}
\newline
...
Gerade bei stationären Texturen erreicht diese Methode gute Ergebnisse.{[QQQ]}
...
\begin{figure}[H]
    ...
    \caption{(a) Zufällige Patch Anordnung, (b) Patches eingeschränkt durch Nachbarn, (c) Minimal Fehler Randschnitt. Nach Alexei A. Efros und William T. Freeman. [QQQ]}%
\end{figure}
...
Gerade bei PCG {(Procedural-Content-Generation)} kann die Pixelgenauigkeit von großen Nutzen sein da dadurch Abgrenzungen der Pixel innerhalb des Output-Images klar definiert sein können. {[HHH]}
...
Jedes lokale Muster ist immer im Input wiederzufinden {(siehe Abbildung 3.1)}. {[HHH][III][JJJ]}
...
ein physikalischer Prozess und spiegelt lediglich eine Aktualisierung unserer Informationen über das System wider} {[KKK, S.5, 2.2 The wave function]}
...
Für solche Probleme finden sich oft bei der Künstlichen Intelligenz und aus dem Operations Research. {[LLL]}
...
nennt sich Wave-Function-Collapse. {[KKK]}
...
Andere Algorithmen verwenden zusätzliche Heuristiken abgesehen von den bereits bekannten constraints, um die Möglichkeit eines ungültigen Zustandes zu reduzieren. {[HHH]}
...
\begin{figure}[H]
    ...
    \caption{(a) Input Struktur, (b) Alexei A. Efros and Thomas K. Leung 1999, (c) Kwatra et al., 2005. [III]}%
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{(a) Input Struktur, (b) Alexei A. Efros and Thomas K. Leung, (c) Paul Merrell Model Synthese. [III]}%
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{(a) 3D-Modell in Komponenten aufgeteilt, (b) Komponente nahtlos zusammengesetzt, (c) Komponente halten Adjazenz-Bedingung nicht ein. [III]}%
\end{figure}
...
Jedoch kann ab diesen Punkt einfach die Model Synthese verwendend werden. {[NNN]}
...
Das hat den Vorteil das die Patches ganz einfach aus dem Input errechnet werden können und der Output sieht dem Input ähnlicher da die Patches enger aneinander liegen. {[NNN]}
...
Sobald ein Feld einen Wert besitzt, dann werden in allen benachbarten Zellen die Patches entfernt, die nicht nutzbar sind. {[HHH]}
...
{(Siehe z.B. Offset $(-1,0)$. Dort sind nur Patches möglich die Rechts 2 weiße Pixel besitzen damit sie sich nahtlos mit den 2 weißen Pixel bei $(0,0)$ überlappen können)}{[HHH]}
...
\begin{figure}[H]
    ...
    \caption[test]{(a) Output mit $N = 2$, (b) Output mit $N = 3$, (c) Output mit $N = 4$. Beispiele generiert mit Kchapelier's Overlapping Model.\footnote[3]{\url{http://www.kchapelier.com/wfc-example/overlapping-model.html}}}
\end{figure}
...
dafür aber der Abschluss aufgrund von komplexen constraint solving Algorithmen nicht gleichmäßig entsteht. {[HHH]}
...
\newline
\url{https://twitter.com/dwtw/status/810166761270243328} [HHH]}
...
\begin{figure}[H]
    ...
    \caption{Die nächsten Zyklen für das Red-Maze Input. {[HHH]}}%
\end{figure}
...
\footnote[6]{Aus Forums-Korrespondenz \url{https://forums.somethingawful.com/showthread.php?threadid=3563643&userid=68893&perpage=40&pagenumber=23\#post467126402}} {[HHH]}

\begin{figure}[H]
    \centering
    \frame{\includegraphics[width=1\linewidth]{images/caves-of-qud.jpg}}%
    \caption{Zwei Spielbare Karten von \textit{Caves of Qud.} {[HHH]}}%
\end{figure}
...
\iffalse
{[AAA]} \url{http://rajvishah.weebly.com/uploads/6/3/0/9/6309814/texture_synthesis_final_report.pdf}\\

BIBTEX:
@misc{Gomathi Shah 2009,
...
{[BBB]} \url{https://www2.eecs.berkeley.edu/Research/Projects/CS/vision/papers/efros-iccv99.pdf}\\
...
{[EEE]} \url{https://www.cns.nyu.edu/heegerlab/content/publications/Heeger-siggraph95.pdf}\\
...
{[QQQ]} \url{https://people.eecs.berkeley.edu/~efros/research/quilting/quilting.pdf}\\

@inproceedings{10.1145/383259.383296,
...
{[HHH]} \url{https://adamsmith.as/papers/wfc_is_constraint_solving_in_the_wild.pdf}\\
...
{[III]} \url{https://paulmerrell.org/wp-content/uploads/2021/06/thesis.pdf}\\
...
{[JJJ]} \url{https://github.com/mxgmn/WaveFunctionCollapse}\\
...
{[LLL]} \url{http://cse.unl.edu/~choueiry/Documents/Lecoutre-eBook-Constraint%20Networks.pdf}\\
...
{[NNN]} \url{https://paulmerrell.org/wp-content/uploads/2021/07/comparison.pdf}\\
...
{[PPP]} {Joseph Parker, Ryan Jones, and Oscar Morante. 2016. Proc Skater 2016. (2016)}\\
